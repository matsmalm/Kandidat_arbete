...
int main() {
	...
	while(readFromFile() != -1){
		...
		int BREAK = 400;
		int envLoop=0;
		res = fopen("RESULTS-30x30.txt", "a+"); // Open file once, will overwrite each run. "a+" = append, "w" = (over)write
		purPaths = fopen("PATHS-30x30.txt", "a+"); // Open file once, will overwrite each run. "a+" = append, "w" = (over)write
		env = fopen("ENV-30x30.txt", "a+"); // Open file once, will overwrite each run. "a+" = append, "w" = (over)write
		fprintf(env, "%d.x.x.x\n", numMatrices);
		printArea();
		for(envLoop=0;envLoop<2;envLoop++){ // Same area 4 times, with different start positions
			printf("start envLoop\n");
			...
			****/
			...
			for(numPur=Hunters[0];numPur>=2;numPur--){ // Loop from Hunters[1] down to 2 pursuers
				...
				for(sameEnv=0;sameEnv<4;sameEnv++){ // Loop with same parameters 4 times
					...
					printf("Tabu\n");
					int tabuSolution[2*(1+Hunters[0]+Hunters[0]*MAX_SIZE_SOLUTION)];
