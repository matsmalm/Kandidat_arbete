...
int best_step_length=1001; //ändra tre ställen							//VIKTIGT steps får inte va för hög ty 2*(jagare=5)*800 = 8000 < int tabu_solution[80000]
...
#define max_step_minus_in_L_list 440 				
#define allowed_stat_4_loss 2 						
#define max_antal_INcomplete_tabu_solutions 1500
#define TABU_MAX_LIKA 3

#define MAX_TABU_STEPS 650
int best_step_length=1001; //ändra tre ställen ORGINALVÄRDE 1001							
#define START_FROM_THE_BEGINING_AGEN_NUMBER 150 		


#define to_easy_problem_problem_adjustment_set_nr_steps_to 10				
#define to_easy_problem_problem_adjustment_go_agen_nr 1	
#define Mss_L_fuck_up 5000
#define Mss_30 20
...
//VIKTIGT steps får inte va för hög ty 2*(jagare=5)*800 = 8000 < int tabu_solution[2+3*Hunters[0]+2*Hunters[0]*MAX_TABU_STEPS] 
...
void preTabu(struct Node (*NodeMat)[SIZE], int *Hunters, int BREAK, int ROWS, int COLS) { // Do all pre-processing, which is to generate population.
	...
	*/
	...
	tabuMatris = calloc(ROWS, sizeof(int *));
	if(tabuMatris == NULL)
		fprintf(stderr, "tabuMatris out of memory\n");
	int i,j;
	for(i = 0; i < ROWS; i++)
	{
		tabuMatris[i] = calloc(COLS, sizeof(int));
		if(tabuMatris[i] == NULL)
			fprintf(stderr, "tabuMatris[i] out of memory\n");
	}
	tabu_solution = calloc(2+3*Hunters[0]+2*Hunters[0]*MAX_TABU_STEPS, sizeof(int));
	memset(tabu_solution,0,sizeof(tabu_solution));
	returned_Tabu_tabu_solution = calloc(2+3*Hunters[0]+2*Hunters[0]*MAX_TABU_STEPS, sizeof(int));
...
void Tabu(int *tabuSolution) { // Main call function for Tabu Algorithm  annat namn Algorithm_Tabu_Search()
	...
	if(tabu_getS4() == 0){
		...
		int q=2;
		int zz=0;
		for(q=2*(1+returned_Tabu_tabu_solution[0]);q<2+3*tabu_solution[0]+2*tabu_solution[0]*MAX_TABU_STEPS;q++){
	...
	while(GOOO==STOOOP){ // lite av en oändlig while loop
		if(tabu_solution[1]==hundra_raknare1){
			...
			if(tabu_solution[1]==MAX_TABU_STEPS){
				...
				for(q=2+2*tabu_solution[0];q<2+3*tabu_solution[0]+2*tabu_solution[0]*MAX_TABU_STEPS;q++){
					tabu_solution[q]=-1;
				}
				tabu_solution[1]=0;
				//printf("snabb?\n");
		...
		if(tabu_solution[1]==best_step_length){ // om lösnignen nått best_step_length så måste vi ju avbryta
			...
			for(qq=2+2*tabu_solution[0];qq<2+3*tabu_solution[0]+2*tabu_solution[0]*MAX_TABU_STEPS;qq++){
				tabu_solution[qq]=-1;
			}
			*/
		...
		if(tabu_getS4() == 0){
			...
			*/
			...
			if(tabu_solution[1]==0 || tabu_solution[1]==1){ //nödvändig?
				...
				for(qqq=2+2*tabu_solution[0];qqq<2+3*tabu_solution[0]+2*tabu_solution[0]*MAX_TABU_STEPS;qqq++){
					tabu_solution[qqq]=-1;
				}
				break;
			}
			*/
			...
			for(qqqq=2+2*tabu_solution[0];qqqq<2+3*tabu_solution[0]+2*tabu_solution[0]*MAX_TABU_STEPS;qqqq++){
	...
	*/
	...
	else if(returned_Tabu_tabu_solution[1]<=MAX_SIZE_SOLUTION){
	...
	printf( "Using %d steps\n",returned_Tabu_tabu_solution[1]);
	printf("\n");
	tabu_calculateStates(returned_Tabu_tabu_solution);
	int hhhhh=0;
