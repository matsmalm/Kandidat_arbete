...
int main() {
	...
	while(readFromFile() != -1){
		...
		int BREAK = 400;
		int envLoop=0;
		res = fopen("RESULTS-15x15.txt", "a+"); // Open file once, will overwrite each run. "a+" = append, "w" = (over)write
		purPaths = fopen("PATHS-15x15.txt", "a+"); // Open file once, will overwrite each run. "a+" = append, "w" = (over)write
		env = fopen("ENV-15x15.txt", "a+"); // Open file once, will overwrite each run. "a+" = append, "w" = (over)write
		fprintf(env, "%d.x.x.x\n", numMatrices);
		printArea();
