...
#define GENETIC_MAX_STEPS 400 // Maximum number of steps, just to allocate enough memory
...
float GENETIC_CONVERGENCE_PERCENT = 0.75; // fraction of population to be equal to break early.
...
int GENETIC_PURSUERS = 0; // Only a temporary value.
int GENETIC_GENERATIONS = 0; // Only a temporary value.
float GENETIC_CONVERGENCE_PERCENT = 0.98; // fraction of population to be equal to break early.
...
void preGenetic(struct Node (*NodeMat)[SIZE], int *Hunters, int BREAK, int ROWS, int COLS) { // Do all pre-processing, which is to generate population.
	Population = calloc(GENETIC_POPULATION_MAX_SIZE, sizeof(struct Chromosome));
	if(Population == NULL)
		fprintf(stderr, "Population out of memory\n");
	New_Population = calloc(GENETIC_POPULATION_MAX_SIZE, sizeof(struct Chromosome));
	...
	GENETIC_POPULATION_SIZE = GENETIC_MIN_POP_SIZE; // Only a temporary value.
	GENETIC_STEPS = GENETIC_MAX_STEPS;
	for(i = 0; i < GENETIC_POPULATION_SIZE; i++){ // Reset all genes.
		...
		Population[i].inS4=ROWS*COLS; // Reset fitness value (S4), just set to high value
...
void getRandom(struct Gene *g, int from){ // Generate random step-sequence from a given node
	int stepnr=0, nextStep;
	struct Node *current = &NodeMatrix[(*g).start[0]][(*g).start[1]];
	for(stepnr = 0; stepnr < GENETIC_STEPS; stepnr++){
		while(1==1){
			if(stepnr<from)
