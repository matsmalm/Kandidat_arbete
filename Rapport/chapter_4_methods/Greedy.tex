...
\\The algorithm is written in an itterative way, and a overview is given by the flow chart in figure \ref{flowchart greedy} In each itteration the algorithm finds and executes the best move for each pursuer. To describe how an iteration is executed an example will now be illustrated.


\begin{enumerate}
\item{} For the first itteration the input is the starting positions of the pursuers. 
...
\item{}Other areas.
\end{itemize}
A boundry to an area is by construction seen by some pursuer. If the area can be seen from a boundry we can usually extend this boundry so that all tiles from which the whole area can be seen form the ``extended boundry''.  
